pub async fn client() -> Result<(), String> {
    let client = tokio::net::TcpStream::connect("localhost:8000").await.map_err(|_| "failed to connect")?;
...
pub async fn connect_server() -> Result<(), String> {
    let listener = tokio::net::TcpListener::bind("localhost:8000").await.map_err(|_| "failed to bind")?;
